...
class AbstractTableForm : AbstractBaseForm{
    ...
    protected List<ArrayList> ress; 


    public AbstractTableForm(List<ArrayList> ress): base() {
    ...
    public override void BtGetData_Click(object sender, EventArgs e){
        foreach(var i in this.ress){
            var arr = i.ToArray().Select(
                (object? i) => i == null ? "NULL" : i.ToString()!

            ).ToArray();
            this.DataGrid.Rows.Add( arr == null ? [] : arr ); 
        }
    }


    public void DataGridSetColumn(bool onlyread, params string[] strings){
    ...
    public void DataGridSetColumn(params string[] strings){ this.DataGridSetColumn(true, strings); }
